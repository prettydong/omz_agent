...
inline constexpr std::size_t kWorkerProtocolVersion = 1;
...
struct WorkerEvent {
  WorkerEvent() = default;
  WorkerEvent(WorkerEventType event_type, std::string event_agent = {},
              std::string event_tool = {}, std::string event_purpose = {},
              std::string event_content = {}, std::string event_error = {},
              core::ModelUsage event_usage = {})
      : type(event_type), agent(std::move(event_agent)),
        tool(std::move(event_tool)), purpose(std::move(event_purpose)),
        content(std::move(event_content)), error(std::move(event_error)),
        usage(std::move(event_usage)) {}
...
[[nodiscard]] core::Result<WorkerRequest>
parse_worker_request(std::string_view json_line);
