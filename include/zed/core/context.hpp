...
public:
    ...
    virtual Result<ContextDecision> decide(
...
public:
    ...
    virtual Result<bool> needs_compaction(
        std::span<const Message> messages,
        const ContextLimits& limits) const = 0;

    virtual Result<ContextWindow> build(
        std::span<const Message> messages,
...
public:
    explicit BasicContextManager(
        TokenEstimator& estimator,
        ContextController* controller = nullptr);

    Result<bool> needs_compaction(
        std::span<const Message> messages,
        const ContextLimits& limits) const override;

    Result<ContextWindow> build(
        std::span<const Message> messages,
        const ContextLimits& limits,
        CancellationToken cancellation) override;

private:
    Result<ContextWindow> build_deterministically(
        std::span<const Message> messages,
