...
Result<TokenCount> available_tokens(const ContextLimits& limits) {
    ...
    return Result<TokenCount>::success(
        limits.max_context_tokens - limits.reserved_output_tokens);
}

}  // namespace

Result<TokenCount> ApproximateTokenEstimator::estimate(
...
BasicContextManager::BasicContextManager(
    TokenEstimator& estimator,
    ContextController* controller)
    : estimator_(estimator), controller_(controller) {}

Result<bool> BasicContextManager::needs_compaction(
    ...
    const TokenCount trigger = limits.compaction_trigger_tokens == 0
...
Result<ContextWindow> BasicContextManager::build(
    std::span<const Message> messages,
    ...
    if (estimated.value() < trigger && estimated.value() <= available.value()) {
    ...
    if (controller_ != nullptr) {
        ...
        if (decision) {
            ...
            for (const auto& candidate : request.candidates) known_ids.insert(candidate.id);
            ...
            if (!required_missing && !unknown_selected) {
                ...
                if (selected_tokens && selected_tokens.value() <= available.value()) {
                    return Result<ContextWindow>::success({
                        ...
                        {selected_tokens.value(), available.value(), limits.reserved_output_tokens},
                        true,
                    });
                }
            }
...
Result<ContextWindow> BasicContextManager::build_deterministically(
    ...
    for (std::size_t index = 0; index < messages.size(); ++index) {
        ...
        if (used > available.value() ||
            cost.value() > available.value() - used) {
            return Result<ContextWindow>::failure({
