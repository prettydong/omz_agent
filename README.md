...
Agent Loop 根据 Provider 的终止状态和待执行工具调用决定是否结束。`response.incomplete`、`response.failed`、缺少终止事件和不一致的工具结束状态都会显示为错误。默认指令要求模型使用工具完成并验证工作；不会再根据回复中的“正在构建”或 “working on it”等关键词额外请求模型或判定失败，避免误伤翻译和引用。
...
`read` 和 `edit` 只读取普通文件，以 4 KiB 分块检查取消，拒绝 FIFO 等特殊文件；读取错误明确返回失败。`edit` 在读取阶段实施大小限制。

`grep` 分块扫描，默认每行最多 256 KiB、总扫描最多 64 MiB、最多检查 100,000 个目录项，扫描时限默认 30 秒；这些内部限制来自 `ToolLimits`。超限、超时或权限/遍历/读取失败返回带 `is_error` 的不完整搜索结果，不能视为完整的“无匹配”。结果数量最多 10,000，调用参数不能扩大输出字节上限。

工具数值参数必须是范围合法的 JSON 整数，不接受浮点数、字符串或负数。Bash 超时及 Bash/Grep 输出上限中的过大正整数在转换前限幅。所有工具结果经 UTF-8 清洗后最多 256 KiB（包含截断标记），写入 Session 前再次限长，保护持久化和下一轮模型上下文。
