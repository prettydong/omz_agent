...
int main() {
  ...
  zed::lsp::ClangdClient clangd({workspace, "/usr/bin/false", {}});
  {
    ...
    const auto app_script = client.Get("/app.js");
    assert(app_script && app_script->status == 200);
    assert(app_script->body.find("highlightCodeBlocks") != std::string::npos);
    assert(app_script->body.find("languageFromPath") != std::string::npos);
    const auto stylesheet = client.Get("/style.css");
    assert(stylesheet && stylesheet->status == 200);
    assert(stylesheet->body.find(".tok-keyword") != std::string::npos);
    assert(stylesheet->body.find("pre.syntax-highlighted::before") !=
           std::string::npos);
