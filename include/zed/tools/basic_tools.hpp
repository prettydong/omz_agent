...
public:
    WorkspaceToolBase(std::filesystem::path workspace_root, ToolLimits limits = {});

protected:
    core::Result<std::filesystem::path> resolve_path(std::string_view path) const;

    [[nodiscard]] const std::filesystem::path& workspace_root() const { return workspace_root_; }
...
public:
    ...
    core::Result<core::ToolResult> execute(
        const core::ToolCall& call,
